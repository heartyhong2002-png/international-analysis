...
- **이슈 21개 전체**(북미3/남미3/유럽3/중동3/아프리카3/아태6)에 대한 규칙 기반 태깅 + AllSides
  편향 태깅 + LLM 톤 분류 파이프라인이 코드상으로는 다 갖춰짐(18번 항목). 다만 북미 3개 외
  나머지 18개는 아직 실제 RSS 데이터로 태깅 결과를 검증한 적은 없음.
- 2차 검수(사람이 표본 검수)를 북미 3개 이슈 기준으로 한 사이클 완료함 — 일치율 60%(3/5),
  `data/review_log.csv`에 기록됨. 6개 언어 교차검증까지 포함하면 총 2차례 검수 사이클 진행(10번,
  15번 항목).
- 톤 분류 프롬프트는 3차까지 개선 완료 — 영어/한국어/중국어/일본어는 안정적으로 동작하고,
  아랍어(Jais)/러시아어(vikhr)는 모델 자체의 한계로 확정, 표본 검수 비율을 높이는 식으로 대응
  하기로 함(16번~17번 항목).
...
- 검수 워크플로 자체(엑셀에서 `tag_status=matched` 필터 → `human_label`/`correction_note`만
  ...
  이번에 21개로 늘었으니 슬슬 필요해질 수 있음.
- **(③번 컨트롤타워 트랙 추가, 19번 항목)** 현지언론/전문가분석 소스 레이어
  프로토타입(`scripts/prototype_local_expert_sources.py`)이 별도 파일로 만들어져
  있음 — 이 파일(`prototype_all_in_one.py`)의 `tag_article`/`OutletBiasLookup`/
  `MODEL_BY_LANGUAGE`/`_call_ollama`를 그대로 import해서 재사용하고, 이 파일은
  직접 건드리지 않음. 자체 self-test는 통과했지만 실제 Ollama(`--with-llm`)
  검증과 이 파일로의 병합은 아직 안 됨. 자세한 내용은 19번 항목과
  `LOCAL_MEDIA_EXPERT_INTEGRATION_HANDOFF.md` 참고.

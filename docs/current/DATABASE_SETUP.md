# MySQL 데이터베이스 통합 및 조기경보 증거 계층 — 작업 요약

> **현재 역할 (2026-09-23 업데이트)**: MySQL은 단순 CSV 통합 저장소가 아니라,
> 조기경보의 근거·점수 산출·경보 이력·사람 검수 결과를 보존하는 증거 계층입니다.
> 대시보드는 원천 테이블을 임의로 조합해 경보를 만들지 않고, 아래의 조기경보 뷰를
> 읽습니다. 이 문서의 초기 "포트폴리오" 설명은 배경 기록이며 현재 방향은
> `PROJECT_CONTEXT.md`와 `docs/current/VALIDATION_PLAN.md`를 우선합니다.
...
원천 수집 데이터는 경보 판단과 분리합니다. 점수 계산기(추후 구현)가 아래 인터페이스에
기록하면, 같은 경보를 나중에 재현하고 오탐·미탐을 감사할 수 있습니다. 수집기와 대시보드는
이 스키마를 직접 구현하지 않아도 됩니다.

| 테이블 | 한 행의 의미 | 보존 원칙 |
|---|---|---|
| `scoring_runs` | 한 번의 점수 계산 실행과 규칙/설정 버전 | 실행마다 추가 |
| `risk_signal_scores` | 실행·이슈·신호 구성요소별 원시값/가중치/기여도 | 기존 행 덮어쓰기 금지 |
| `signal_gap_snapshots` | 이슈·실행 시점의 공식·독립·대중·시장 신호 괴리 | 실행마다 추가 |
| `alert_events` | Alert Level의 발생·변경·해제 이벤트 | append-only |
| `alert_event_evidence` | 경보와 점수/원천 문서/인용문의 연결 | 경보별 근거를 모두 연결 |
| `alert_validation_audit` | 사람 검수·백테스트의 확정/오탐/미탐/근거부족 판정 | `alert_event_id` 없이 미탐도 기록 가능 |
| `data_quality_audit` | 중복·인코딩 정리의 대상, 사유, 백업 경로 | 삭제 전후 모두 감사 |

`risk_signal_scores`의 `signal_family`는 `official`, `independent`, `public`,
`market`, `trade`, `llm`처럼 출처 계층을, `metric_name`은 실제 측정값을 표현합니다.
`alert_level`과 `verdict`는 자유 텍스트가 아니라 아래 어휘로 통일합니다.

```
alert_level: WATCH | WARNING | CRITICAL
lifecycle_status: active | resolved | superseded | held_for_review
event_type: raised | level_changed | resolved | superseded
verdict: confirmed | false_alarm | missed_signal | insufficient_evidence | pending_review
```

새 조기경보 테이블의 모든 시각은 UTC `DATETIME`입니다. 기존 원천 테이블의 문자열 날짜는
호환성을 위해 유지하되, 신규 설계에 복제하지 않습니다.

### 5.0.1 점수 계산기용 DB 쓰기 계약

1. `scoring_runs`에 `run_key`, `scoring_version`, `config_hash`, 분석 기간을 먼저 기록한다.
2. 같은 `scoring_run_id`에 이슈별 `risk_signal_scores` 구성요소와
   `signal_gap_snapshots`를 기록한다.
3. Alert Level 변화가 실제로 결정된 경우에만 `alert_events`를 추가하고,
   그 경보를 만든 문서·인용문·점수 행을 `alert_event_evidence`로 연결한다.
4. 사람 검수나 과거 사례 백테스트 후 `alert_validation_audit`에 판정을 추가한다.

점수 산정식과 임계값은 `scoring_version`/`config_hash`로 식별합니다. SQL 뷰에 임계값을
고정하지 않습니다. `REPLACE INTO`는 기존 행을 삭제 후 재삽입하므로 이 증거 계층에는
사용하지 않습니다.
...
| 뷰 | 화면 용도 |
|---|---|
| `v_dashboard_current_alerts` | 이슈별 최신 점수, 경보 단계, 근거 수, 신호 괴리, 검수 필요 상태 |
| `v_alert_evidence_trace` | 선택한 경보의 원천 링크, 인용문, 점수 구성요소 추적 |
| `v_signal_gap_trend` | 공식·독립·대중·시장 신호 괴리의 시계열 |
| `v_alert_validation_summary` | Alert Precision, 오탐·미탐·근거부족·검수 현황 |

기존 `v_issue_geopolitical_risk_matrix`는 대중 관심도와 정부 발표의 **보조 진단**으로
계속 유지합니다. 이 뷰는 임계값이 SQL에 고정되고 서로 다른 수집 시점의 값을 비교하므로,
정식 Alert Level을 생성하는 용도로 쓰지 않습니다.

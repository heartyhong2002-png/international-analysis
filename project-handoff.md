...
**개발자:** 홍준기
**목적:** 교과목/졸업 과제물 및 포트폴리오용 프로젝트
**핵심 스토리:** 제한된 하드웨어(16GB RAM, GPU 없음) 환경에서 다국어
오픈소스 LLM을 활용해 국제정세와 공급망 리스크 신호를 분석하는 조기경보
시스템을 구축 — "예측 가능성의 한계를 인식하고, 검증 가능한 문제 정의로
피벗한 과정"과 "제약을 창의적으로 극복하는 엔지니어링 능력"을 함께 증명하는 것이 목표.
...
- 2026-09-23 [트랙③, 프로젝트 정체성 및 문서 체계 재정리] **예측 중심 접근을 폐기 이력으로 보존하고, 현재 계획을 조기경보 시스템으로 분리 문서화**:
  1. **결정 배경**:
     - 초기 목표였던 "국제정세 사건 예측"은 미래 사건의 Ground Truth 정의, 장기 검증 필요성, 오탐/미탐 해석의 어려움 때문에 졸업작품 범위에서 방어하기 어렵다고 판단함.
     - 대신 기존 수집·분석 자산을 폐기하지 않고, 관측 가능한 위험 신호를 탐지하는 **AI 기반 국제정세 및 공급망 리스크 조기경보 시스템**으로 재정의함.
  2. **문서 구조 정리**:
     - `docs/history/PROJECT_EVOLUTION_TIMELINE.md`: 예측 구상 → 대시보드 정리 → 챗봇 검토·보류 → 조기경보 피벗의 전체 타임라인.
     - `docs/history/PREDICTION_APPROACH_RETIRED.md`: 예측 접근을 어느 정도 구현했고 왜 폐기했는지 정리.
     - `docs/history/CHATBOT_PIVOT_RETIRED.md`: 챗봇 서비스 전환 검토와 보류 이유 정리.
